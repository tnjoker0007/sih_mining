...
// Forward alert events to Module 3 (Command Center Webhook)
async function forwardAlertToModule3(alertData) {
  const webhookUrl = 'http://localhost:5000/api/alerts/miner';
  try {
    ...
    console.log(`[Integration] Alert forwarded to Module 3:`, result.success ? `Success (ID: ${result.alert_id})` : `Failed: ${result.message}`);
  } catch (err) {
    console.error(`[Integration Error] Failed to forward alert to Module 3:`, err.message);
  }
}
...
app.post('/api/bands/data', (req, res) => {
  ...
  if (sos) {
    ...
    if (!activeSos) {
      ...
      // Forward alert to Module 3 Command Center
      forwardAlertToModule3({
        band_id: band_id,
        miner_id: minerId,
        event_type: 'SOS',
        heart_rate: 142,
        ...
        zone_id: zoneId,
        sos_id: newSos.id,
        notes: `EMERGENCY: SOS Button pressed on smart band ${band_id} for miner ${miner ? miner.name : 'Unknown'}.`
      });
    }
  }
...
app.post('/api/fall-events/trigger', (req, res) => {
  ...
  // Forward fall SOS to Module 3 Command Center
  const band = db.bands.find(updatedFall.band_id);
  const minerId = band ? band.miner_id : null;
  const miner = minerId ? db.miners.find(minerId) : null;
  const zoneId = miner ? miner.zone_id : (updatedFall.zone_id || 'ZONE_B');

  // Find the created SOS event to get its ID
  const activeSos = db.sos.active().find(e => e.band_id === updatedFall.band_id);

  forwardAlertToModule3({
    band_id: updatedFall.band_id,
    miner_id: minerId,
    event_type: 'FALL_DETECTED',
    heart_rate: 138,
    spo2: 92,
    impact_g: updatedFall.acceleration || 5.4,
    immobile_duration_sec: 180,
    zone_id: zoneId,
    sos_id: activeSos ? activeSos.id : null,
    notes: `CRITICAL ALARM: Miner Fall & Inactivity detected on Wearable ${updatedFall.band_id} (${miner ? miner.name : 'Unknown'}).`
  });
...
app.post('/api/sos', (req, res) => {
  ...
  // Forward manual SOS to Module 3 Command Center
  forwardAlertToModule3({
    band_id: finalBandId,
    miner_id: finalMinerId,
    event_type: 'SOS',
    heart_rate: 135,
    spo2: 92,
    impact_g: 1.0,
    zone_id: finalZoneId || 'ZONE_A',
    sos_id: sos.id,
    notes: details || `Manual SOS triggered by control room for ${finalMinerId}.`
  });
  ...
  db.bands.all().forEach(band => {
    if (band.online) {
      ...
      if (now - lastActiveTime > OFFLINE_TIMEOUT) {
        ...
        // Forward communication loss to Module 3 Command Center
        forwardAlertToModule3({
          band_id: band.band_id,
          miner_id: band.miner_id,
          event_type: 'COMMUNICATION_LOSS',
          zone_id: miner ? miner.zone_id : 'ZONE_A',
          notes: `COMMUNICATION LOSS: Wearable ${band.band_id} (${minerName}) offline.`
        });
