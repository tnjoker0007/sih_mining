...
export default function Sidebar({ activeTab, setActiveTab }) {
  ...
  const navItems = [
    ...
    { id: 'analytics', label: 'Sentinel Analytics', icon: Cpu, badge: null },
  ...
  return (
    <aside className="w-60 border-r border-[#1f2d52] bg-[#0d1326] flex flex-col justify-between p-3 min-h-[calc(100vh-53px)] select-none">
      <div className="space-y-1">
        <div className="px-2.5 py-1.5 text-[11px] font-mono uppercase tracking-wider text-[#8c9bb4] font-semibold">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded font-sans font-bold text-xs transition-all ${
                isActive
                  ? 'bg-gradient-to-r from-[#4facfe] to-[#00f2fe] text-[#060913] shadow-lg shadow-cyan-500/10'
                  : 'text-gray-300 hover:bg-[#151d36] hover:text-white'
              }`}
            >
              <div className="flex items-center gap-2.5">
                <Icon className={`w-4 h-4 ${isActive ? 'text-[#060913]' : 'text-gray-400'}`} />
                <span>{item.label}</span>
              </div>

              {item.badge && (
                <span className={`px-1.5 py-0.2 rounded text-[10px] font-mono font-bold text-white ${item.badgeColor || 'bg-[#151d36]'}`}>
                  {item.badge}
                </span>
              )}
            </button>
      ...
      <div className="pt-3 border-t border-[#1f2d52] space-y-2">
        <div className="p-2.5 rounded bg-[#060913] border border-[#1f2d52] text-[11px] font-mono">
          <div className="flex items-center justify-between text-gray-400 mb-1">
            <span className="flex items-center gap-1.5">
              <RadioTower className="w-3.5 h-3.5 text-[#00f2fe]" />
              <span>Mod 1 AI:</span>
            </span>
          ...
          <div className="flex items-center justify-between text-gray-400">
            ...
            <span className="text-[#39ff14] font-bold">{miners.length} Active</span>
          </div>
        </div>
