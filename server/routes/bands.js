...
router.post('/broadcast', (req, res) => {
  ...
  // Forward broadcast to Module 2 Wearable Band server so smartwatch emulator displays it
  fetch('http://localhost:3000/api/bands/broadcast', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      target_type: target_type === 'ALL' ? 'all' : (target_type === 'ZONE' ? 'zone' : 'band'),
      target_id: target_id || 'ALL_ZONES',
      message_text: message,
      severity: alert_level === 'EMERGENCY_EVACUATE' ? 'CRITICAL' : 'WARNING'
    })
  }).catch(err => console.error('[Integration Error] Failed to forward broadcast to Module 2:', err.message));
