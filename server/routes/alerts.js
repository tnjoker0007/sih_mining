...
router.post('/miner', (req, res) => {
  const {
    band_id,
    miner_id,
    event_type, // 'SOS', 'FALL_DETECTED', 'ABNORMAL_VITALS', 'GEOFENCE_BREACH', 'COMMUNICATION_LOSS'
    heart_rate,
    spo2,
    impact_g,
  ...
  const newAlert = {
    ...
    title: event_type === 'SOS' 
      ...
      ? `⚠️ Miner Communication Loss: ${miner ? miner.name : 'Miner'}`
  ...
  if (miner) {
    db.update('miners', miner.id, {
      status: event_type === 'SOS' ? 'SOS' : (event_type === 'FALL_DETECTED' ? 'FALL_DETECTED' : event_type === 'COMMUNICATION_LOSS' ? 'OFFLINE' : 'WARNING'),
      vitals: {
        ...miner.vitals,
        heart_rate: heart_rate || miner.vitals.heart_rate,
        spo2: spo2 || miner.vitals.spo2,
        impact_g: impact_g || miner.vitals.impact_g,
        motion_state: event_type === 'FALL_DETECTED' ? 'MOTIONLESS' : event_type === 'COMMUNICATION_LOSS' ? 'OFFLINE' : miner.vitals.motion_state
      }
    });
  }
...
router.put('/:id/acknowledge', (req, res) => {
  ...
  // Sync acknowledgement back to Module 2 if from Wearable SOS
  if (alert.source === 'MODULE_2_BAND' && alert.data_payload && alert.data_payload.sos_id) {
    fetch('http://localhost:3000/api/sos/acknowledge', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: alert.data_payload.sos_id, user: user_name || 'Operator' })
    }).catch(err => console.error('[Integration Error] Failed to sync ack to Module 2:', err.message));
  }
...
router.put('/:id/status', (req, res) => {
  ...
  if (status === 'RESOLVED') {
    ...
    // Sync resolution back to Module 2 if from Wearable SOS
    if (alert.source === 'MODULE_2_BAND' && alert.data_payload && alert.data_payload.sos_id) {
      fetch('http://localhost:3000/api/sos/resolve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: alert.data_payload.sos_id })
      }).catch(err => console.error('[Integration Error] Failed to sync resolve to Module 2:', err.message));
    }
  }
